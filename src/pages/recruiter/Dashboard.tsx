...
export default function RecruiterDashboard() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'closed': return 'text-white border-zinc-700 bg-zinc-900/20';
    ...
    <div className="min-h-screen bg-white dark:bg-cyber-dark text-zinc-700 dark:text-zinc-300 font-sans antialiased selection:bg-accent-pink selection:text-white w-full">
      ...
      <main className="mx-auto px-6 lg:px-16 py-12 pb-20">
        ...
        <div className="mb-12 space-y-2">
          ...
          <p className="text-white text-sm">Manage job postings, track applicants, and streamline hiring workflows in real-time</p>
        </div>
              ...
              <tbody>
                {loading
                  ...
                  : recruiterJobs?.data?.map((job, idx) => (
                    <tr
                      key={job.id}
                      className={`border-b border-panel-border hover:bg-white dark:bg-cyber-dark/50 transition-colors ${idx % 2 === 0 ? "bg-white dark:bg-cyber-dark/30" : ""
                        }`}
                    >
                      <td className="px-6 py-4">
